...
#By: Dovvy Pacamalan
# Notes:
# Works only for Firefox & Google Chrome Browsers
# How to Play
# Use Arrow UP, Left & Right to control the ship
# Press Spacebar to fire 
# Click the Play button above to start the Game.
# Have Fun
...
class Ship:
    ...
    def shoot(self):
        ...
        missile_pos = [missile_direction[0] * self.radius + self.pos[0], missile_direction[1] * self.radius + self.pos[1]]
        missile_vel = (self.vel[0] + missile_direction[0] * 5),( self.vel[1] + missile_direction[1] * 5 ) 
...
def draw(canvas):
    global a_time,lives,score,started
    
    # animiate background
    a_time += 1
    wtime = (a_time / 4) % WIDTH
...
def rock_spawner():
    global rock_group
    if started:
        rndpos = [random.randrange(0,WIDTH), random.randrange(0,HEIGHT)]
        for rock in rock_group:
            while dist(rndpos, my_ship.pos) <= rock.radius + my_ship.radius:
                rndpos = [random.randrange(0,WIDTH), random.randrange(0,HEIGHT)] 
                
        if len(rock_group) < 12:    
            #rndvel = [random.random() * .6 - .3, random.random() * .6 - .3]   
            #velocity based on score 
            rndvel = [random.random()*(score*.08) * .6 - .3, random.random() *(score*.08) * .6 - .3]
            rndang = random.random() * .2 - .1
            a_rock = Sprite([rndpos[0], rndpos[1]], rndvel, .05, rndang, asteroid_image, asteroid_info)  
...
frame.set_mouseclick_handler(reset_game)
